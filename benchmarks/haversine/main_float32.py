...
def gen_data(lat, lon, scale=10):
    ...
    new_lat = np.array(new_lat, dtype=np.float32)
    new_lon = np.array(new_lon, dtype=np.float32)
...
def read_data():
    start = time.time()
    new_lat = np.fromfile(LATS_NAME, dtype=np.float32)
    new_lon = np.fromfile(LONS_NAME, dtype=np.float32)
    end = time.time()
...
LATS_NAME = LATS_NAME + str(args.scale) + "_float32"
LONS_NAME = LONS_NAME + str(args.scale) + "_float32"
